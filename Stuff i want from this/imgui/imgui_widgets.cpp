...
void ui::TabButton(const char* label, int* selected, int num, int total) {
    ...
    if (*selected != num) 
	{
		...
		window->DrawList->AddLine(pos + ImVec2(size.x, 0), pos + ImVec2(size.x, size.y), ImColor(45 / 255.f, 45 / 255.f, 45 / 255.f, g.Style.Alpha));
    }
    else 
	{
		//upper
		window->DrawList->AddLine(pos + ImVec2(0, 2), pos + ImVec2(size.x, 2), ImColor(45 / 255.f, 45 / 255.f, 45 / 255.f, g.Style.Alpha));
		//lower
		window->DrawList->AddLine(pos + ImVec2(0, 89), pos + ImVec2(size.x, 89), ImColor(45 / 255.f, 45 / 255.f, 45 / 255.f, g.Style.Alpha));
    }
...
bool ui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
        window->DrawList->AddRectFilled(pos + ImVec2(2, 2), pos + ImVec2(10, 10), ImColor(c_config::get()->c["menu_color"][0], c_config::get()->c["menu_color"][1], c_config::get()->c["menu_color"][2]));
    else {
        if (hovered)
...
bool ui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power, int remove_from_fmt)
{
    ...
    window->DrawList->AddRectFilled(slider_bb.Min, ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)), slider_bb.Max.y), ImColor(c_config::get()->c["menu_color"][0], c_config::get()->c["menu_color"][1], c_config::get()->c["menu_color"][2]));
...
bool ui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    window->DrawList->PushClipRect(bb.Min, bb.Max, true);
    window->DrawList->AddText(bb.Min + ImVec2(8, size.y / 2 - label_size.y / 2 + 2), selected ? ImColor(c_config::get()->c["menu_color"][0], c_config::get()->c["menu_color"][1], c_config::get()->c["menu_color"][2]) : ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), label);
    window->DrawList->PopClipRect();
